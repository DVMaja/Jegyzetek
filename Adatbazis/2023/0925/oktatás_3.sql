...
create table diák
(
azon int identity (101,1),--auto increment
...
(
osztály char(2),
nap tinyint,--nem dátum, hanem a hét hanyadik napja
óra tinyint,
tant char(5),
...
alter table órarend -- oszlop hozzáadása
add terem int
-- vagy drop vagy deledte
alter table órarend
drop column terem
...
select * from órarend
order by 1, 2, 3
--a kulcs szerint az osztály nem is tudna ütközni
alter table órarend -- oszlop hozzáadása
add terem int
...
update órarend set
terem=1
--where osztály like '1%' --% vagy _ jele a maszkolás
where terem is null
--terem ütközzön adott napon adott órájában
--egy UK biztosítja nap óra terem  (nemclusteres indexként)
--ide kellene egy adag sql, de nem tudom honnan szedjem elõ

--a tanár se ütközzön az adott nap adott órájában, már egy igazi constraintet igényel
